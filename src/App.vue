...
async function addToCart(classInfo) {
    ...
    if (classInfo.availablePlaces > 0 && !cart.value.some(item => item._id === classInfo._id)) {
        ...
        await fetch(`https://fs-cw-express-env.eba-n2trmxhn.eu-west-2.elasticbeanstalk.com/lesson/${classInfo._id}`, {
            method: 'PUT',
...
const fetchCart = async () => {
  ...
  const response = await fetch(`https://fs-cw-express-env.eba-n2trmxhn.eu-west-2.elasticbeanstalk.com/cart?userId=${userId}`);
...
async function saveCartToBackend() {
    const userId = localStorage.getItem('userId');
    await fetch('https://fs-cw-express-env.eba-n2trmxhn.eu-west-2.elasticbeanstalk.com/cart', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
...
const fetchClasses = async () => {
    ...
    try {
        ...
        const response = await fetch(`https://fs-cw-express-env.eba-n2trmxhn.eu-west-2.elasticbeanstalk.com/lessons?${params}`);
